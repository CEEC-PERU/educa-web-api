        ...
        socket.on('module', async (data) => {
            ...
            try {
                await sequelize.transaction(async (transaction) => {
                    // Verificar si ya existe un progreso de módulo para el usuario
                    const existingProgress = await getUserModuleProgressByModuleAndUser(module_id, user_id, { transaction });
                    
                    if (existingProgress) {
                        // Si existe, actualizar el progreso de módulo
                        await updateUserModuleProgressByModuleAndUser(module_id, user_id, { progress, is_completed }, { transaction });
                        console.log("Progreso de módulo actualizado:", { module_id, progress, user_id, is_completed });
                    } else {
                        // Si no existe, crear un nuevo registro de progreso de módulo
                        await createUserModuleProgress({ user_id, module_id, progress, is_completed }, { transaction });
                        console.log("Nuevo progreso de módulo creado:", { module_id, progress, user_id, is_completed });
                    }

                    // Emitir evento solo después de la transacción exitosa
                    io.emit('module-progress-updated', { module_id, progress, user_id, is_completed });
                });
            } catch (error) {
                console.error('Error al manejar el progreso del módulo:', error);
            }
        });
